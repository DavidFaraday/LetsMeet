...
func saveLikeToUser(userId: String) {
    ...
    if let currentUser = FUser.currentUser() {
        ...
        if !didLikeUserWith(userId: userId) {
            ...
            FirebaseUserListener.shared.updateCurrentUserInFireStore(withValues: [kLIKEDIDARRAY: currentUser.likedIdArray!]) { (error) in
...
func createRecentItems(chatRoomId: String, users: [FUser]) {
    ...
    for user in users {
        memberIdsToCreateRecent.append(user.id)
    }
...
func lookingForGender() -> String {
    return FUser.currentUser()?.lookingFor ?? "Bi"
}

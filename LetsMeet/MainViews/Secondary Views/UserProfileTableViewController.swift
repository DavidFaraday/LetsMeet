...
//  LetsMeet
//
//  Created by David Kababyan on 12/07/2020.
//
...
class UserProfileTableViewController: UITableViewController {
    ...
    @IBAction func likeButtonPressed(_ sender: Any) {
        ...
        if self.navigationController != nil {
            saveLikeToUser(userId: userObject!.id)
            FirebaseListener.shared.saveMatch(userId: userObject!.id)
            showMatchView()
        } else {
            dismissView()
        }
    ...
    private func loadImages() {
        ...
        let avatar = userObject!.avatar ?? UIImage(named: "mPlaceholder")
    ...
    private func updateLikeButtonStatus() {
        likeButtonOutlet.isEnabled = !FUser.currentUser()!.likedIdArray!.contains(userObject!.id)
    }
    ...
    private func goToChat() {
        ...
        let chatView = ChatViewController(chatId: chatRoomId, recipientId: userObject!.id, recipientName: userObject!.username)
